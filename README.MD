...
If browser tests fail with "Playwright not found" or similar errors:

1. **First run takes time** - Playwright downloads browsers on first use (~400MB)
2. **Behind proxy** - Set proxy environment variables:
   ```powershell
