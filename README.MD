...
<p align="center">
  <a href="https://github.com/EvotecIT/PSParseHTML/actions/workflows/powershell-tests-7.yml"><img src="https://github.com/EvotecIT/PSParseHTML/actions/workflows/powershell-tests-7.yml/badge.svg"></a>
...
**PSParseHTML** exposes a suite of PowerShell cmdlets that let you parse, format and optimise web resources right from the shell.  The module currently ships with fourteen cmdlets:
...
Microdata parsing recognizes common schema.org item types so you can easily
process structured data embedded in pages.
...
- `Get-HTMLNetworkLog` - view captured network requests and responses
 - `Get-HTMLResource` - extract script and CSS resources with comments, optionally returning content or saving files
- `Get-HTMLConsoleLog` - retrieve messages printed to the browser console
- `Set-HTMLCookie` - add cookies to the active session
- `Export-BrowserState`/`Import-BrowserState` - save and restore cookies and storage state
...
Export-BrowserState -Session $session -Path 'state.json'
$session = Import-BrowserState -Path 'state.json' -Url 'https://example.com/protected'
...
ConvertFrom-HtmlMicrodata -Content '<div itemscope itemtype="https://schema.org/Product"><span itemprop="name">Paper</span></div>'
```
...
- [Microsoft.Playwright](https://github.com/microsoft/playwright-dotnet) - Apache 2.0 License
- [System.Net.Http](https://learn.microsoft.com/dotnet/api/system.net.http) - MIT License
...
[HtmlTinkerX.HtmlFormatter]::FormatHtml($html)
```
