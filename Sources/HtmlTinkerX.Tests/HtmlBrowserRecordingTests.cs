...
/// <summary>
/// Tests StartRecordingAsync and StopRecordingAsync helpers.
/// </summary>
public class HtmlBrowserRecordingTests {
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public async Task StartStopRecordingAsync_ReturnsFile(bool useSession) {
        ...
        if (useSession) {
            ...
            session = await HtmlBrowser.StartRecordingAsync(new HtmlBrowserSession(playwright.Object, initBrowser.Object, initContext.Object, initPage.Object), outFile);
        } else {
            session = await HtmlBrowser.StartRecordingAsync("https://example.com", outFile);
        }

        string result = await HtmlBrowser.StopRecordingAsync(session);

        Assert.Equal(HtmlUtilities.ResolvePath(outFile), result);
