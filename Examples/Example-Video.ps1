...
$S1 = Start-HTMLVideoRecording -OutFile "$PSScriptRoot\Output\WP1.mp4"

Get-HTMLInteractable -Filter "Media" -IncludeHidden | Format-Table

$S2 = Invoke-HTMLNavigation -Selector "a[href='edit.php?post_type=thegem_gallery']" -Session $Session

$S3 = Stop-HTMLVideoRecording -OutFile "$PSScriptRoot\Output\WP1.mp4"